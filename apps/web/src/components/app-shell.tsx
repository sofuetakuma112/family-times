...
export default function AppShell({ session }: AppShellProps) {
  ...
  useEffect(() => {
    ...
    const nextServer = servers.data[0];
  ...
  useEffect(() => {
    if (!serverId || !channels.data) return;
    ...
    const nextChannel = channels.data[0];
    setChannelId(nextChannel?.id ?? null);
    setChannelName(nextChannel?.channelName ?? null);
  }, [channelId, channels.data, serverId]);
